...
function convertVideo() {
    ...
    result.innerHTML = `
        <div style="background:#e8f5e9;padding:14px;border-radius:var(--radius-md);color:#2e7d32;display:flex;justify-content:space-between;align-items:center;">
            <span>✅ Audio conversion started!</span>
            <button onclick="downloadAudio()" style="background:#2e7d32;color:white;border:none;padding:8px 20px;border-radius:var(--radius-full);cursor:pointer;font-weight:700;">Download Audio</button>
        </div>
    `;
    showToast('🎵 Converting video to audio...');
    playSound('start');
}
...
function exportPDF() {
    try {
        ...
        doc.setFillColor(15, 123, 142);
        doc.rect(0, 0, pageWidth, 45, 'F');
        
        // Gradient overlay
        for (let i = 0; i < 45; i++) {
            doc.setFillColor(15 - i * 0.15, 123 - i * 0.2, 142 - i * 0.15);
            doc.rect(0, i, pageWidth, 0.5, 'F');
        }

        doc.setFontSize(24);
        doc.setTextColor(255, 255, 255);
        doc.text('📚 ' + APP_NAME, pageWidth / 2, 22, { align: 'center' });
        doc.setFontSize(10);
        doc.setTextColor(200, 230, 255);
        const now = new Date();
        doc.text(now.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }), pageWidth / 2, 33, { align: 'center' });
        doc.text('v' + APP_VERSION, pageWidth / 2, 40, { align: 'center' });

        let y = 58;

        // Stats Card
        const total = data.lessons.length + data.assignments.length + data.exams.length;
        const done = data.lessons.filter(l => l.done).length + data.assignments.filter(a => a.done).length + data.exams.filter(e => e.done).length;
        const completion = total > 0 ? Math.round((done / total) * 100) : 0;

        doc.setFillColor(240, 247, 255);
        doc.roundedRect(10, y, pageWidth - 20, 28, 4, 4, 'F');
        doc.setDrawColor(15, 123, 142);
        doc.setLineWidth(0.5);
        doc.roundedRect(10, y, pageWidth - 20, 28, 4, 4, 'S');
        
        doc.setFontSize(12);
        doc.setTextColor(11, 59, 92);
        doc.text(`📊 Stats: ${total} items | ✅ ${done} done | 📈 ${completion}% progress | ⏱️ ${data.timerSessions || 0} sessions`, pageWidth / 2, y + 18, { align: 'center' });
        y += 36;

        // Progress bar
        doc.setDrawColor(200, 200, 200);
        doc.rect(10, y, pageWidth - 20, 5, 'S');
        if (completion > 0) {
            doc.setFillColor(15, 123, 142);
            doc.rect(10, y, ((completion / 100) * (pageWidth - 20)), 5, 'F');
        }
        y += 14;
        ...
        if (data.subjects.length) {
            ...
            doc.autoTable({
                startY: y,
                head: [['#', 'Name']],
                body: data.subjects.map((s, i) => [i + 1, s.icon + ' ' + s.name]),
                theme: 'striped',
                styles: { fontSize: 10, halign: 'left' },
                headStyles: { fillColor: [15, 123, 142], textColor: [255, 255, 255] }
            });
        ...
        if (data.lessons.length) {
            doc.setFontSize(14);
            doc.text('📖 Lessons', 14, y);
            y += 7;
            doc.autoTable({
                startY: y,
                head: [['Title', 'Subject', 'Days', 'Time', 'Status']],
                body: data.lessons.map(l => [
                    l.title,
                    l.subject,
                    l.days ? l.days.join(', ') : '',
                    l.time,
        ...
        if (data.assignments.length) {
            doc.setFontSize(14);
            doc.text('✏️ Assignments', 14, y);
            y += 7;
            doc.autoTable({
                startY: y,
                head: [['Title', 'Subject', 'Deadline', 'Status']],
        ...
        if (data.exams.length) {
            ...
            doc.autoTable({
                startY: y,
                head: [['Title', 'Subject', 'Date', 'Status']],
        ...
        if (data.schedule.length) {
            doc.setFontSize(14);
            doc.text('📅 Schedule', 14, y);
            y += 7;
            doc.autoTable({
                startY: y,
                head: [['Day', 'Subject', 'Time', 'Location']],
        ...
        doc.text(`Exported by ${APP_NAME} v${APP_VERSION} | ${APP_AUTHOR}`, pageWidth / 2, pageHeight - 8, { align: 'center' });

        doc.save(`${APP_NAME}_Report_${now.toISOString().slice(0,10)}.pdf`);
        showToast('✅ PDF exported successfully');
        launchCelebration();
        playSound('end');
    } catch (e) {
        showToast('⚠️ PDF Error: ' + e.message, 'error');
    }
}
